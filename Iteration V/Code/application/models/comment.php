...
class Comment extends CI_Model
{
	...
	public function load_all_comments($picture_id) {
		...
		if ($query->num_rows() > 0) {
			foreach ($query->result() as $comment) {
				$detail = array();
				$detail['pic'] = $this->get_picture_profile($comment->user_id);
	...
	// get picture profile
	private function get_picture_profile($user_id) {
		$this->db->where('user_id', $user_id);
		$query = $this->db->get($this->user_profile_table_name);
		if ($query->num_rows() == 1) {
			$pic = $query->row()->pic_address;
			if ($pic) {
				return $pic;
			}
			else {
